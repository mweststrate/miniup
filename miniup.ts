...
export default function miniup(grammar: string): Grammar;
export default function miniup(grammar: string, input?: string, opts?: IParseArgs):any;
export default function miniup(grammar: string, input?: string, opts?: IParseArgs):any {
	if (input === undefined)
		return Grammar.load(grammar);
...
//	TODO: needed?					delete this.memoizedParseFunctions[func.memoizationId][startpos]
				...
				try {
					...
					else if (this.cleanAST === false && result !== null && !result.$rule && func.ruleName && typeof result === "object")
	...
	public static main() {
		...
		try {
			if (argv.s)
				...
				grammar = Grammar.load(argv.g, "input")
			else
				grammar = GrammarReader.getMiniupGrammar();
		}
		catch (e) {
			handleException(e);
		}
